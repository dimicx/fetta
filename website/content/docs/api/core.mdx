...
Pass a Promise to `revertOnComplete` to restore the original HTML when it resolves.
...
Enable `autoSplit` to re-split text when the container resizes. Call `dispose()` to cleanup observers when done.
